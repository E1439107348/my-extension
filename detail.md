   ...
   +----------+-----------+-----------+
           ...
           +-------------+    +-------------+
               ...
               +-----------------------+
                 |      |      |      |
                 v      v      v      v
               Redis  MySQL  Tracing 外部API
```


### 架构分层说明
- 接入层：`otel`、`logger`、`limiter`、`auth.*`，负责入口治理与可观测性。
- 业务支撑层：`cache`、`redisson`、`message`、`threadPool`，负责性能与并发能力。
- 数据访问层：`mybatis` + `SQL Trace`，负责数据库访问与可观测。
- 出站集成层：`http`、`feign`、`alarm`，负责下游调用和告警通知。
- 基础设施层：`Redis`、`MySQL`、`Tracing Backend`、`第三方服务`。
