...
五、config（摘要）
----------------------------
  1) LoggingAutoConfiguration是在所有打印日志前面添加TraceId，使用方式,在启动文件上面添加：
  @ImportAutoConfiguration(org.example.myextension.config.LoggingAutoConfiguration.class)
  2) myextension.logging.files.url: 配置日志文件存储路径,比如：/Users/qy/tmp/log/application.log


六、OAuth2（Sa-Token）核心使用说明（详细）
-------------------------------------------
